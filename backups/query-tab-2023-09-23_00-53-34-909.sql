...
-- 숙제. 각 호선 별 승하차 인원 수가 가장 많은 역 구하기 (4월 기준)																	
...
FROM
	(
		SELECT
			...
			A.STATN_NM,
			A.HO_LN,
			ROW_NUMBER() OVER (PARTITION BY A.HO_LN ORDER BY SUM(A.NMPR_CNT) DESC) AS RN_HO_LN_NMPR_CNT,
			SUM(A.NMPR_CNT) AS NMPR_CNT
		FROM TB_PBTRNSP A
		WHERE A.STD_MT = '202304'
		GROUP BY A.STATN_NO, A.STATN_NM, A.HO_LN
